...
while True:
    ...
    if (not num_1.isnumeric() and not num_2.isnumeric()) and (not operador == '+' and not operador == '-' and not operador == '*' and not operador == '/'):
        print('Voce precisa digitar um numero no primeiro, outro no segundo valor e um operador valido !!!')
        continue
    if not num_1.isnumeric() and (not operador == '+' and not operador == '-' and not operador == '*' and not operador == '/'):
        print('Digite em valor inteiro no primeiro e um operador valido!!!!')
        continue
    if not num_2.isnumeric() and (not operador == '+' and not operador == '-' and not operador == '*' and not operador == '/'):
